...
#pragma omp parallel for
    for (uint i=0; i < numBlocks; ++i) {
	    ...
	    // goes to k th bucket in block i
            uint k = (keys[i*blockSize+j] >> startBit) & (numBuckets-1);
	    ++blockHistograms[i*numBuckets+k];
	}
    }
...
std::vector<uint> scanGlobalHisto(
    ...
    for (uint i=1; i < numBuckets; i++) {
        cumSum += globalHisto[i-1];
	globalHistoExScan[i] = cumSum;
    }
...
std::vector<uint> computeBlockExScanFromGlobalHisto(
    ...
    // write from the second block
    for (uint j=numBuckets; j<blockExScan.size();j++) {
        blockExScan[j] = blockHistograms[j-numBuckets]+blockExScan[j-numBuckets];
    }
    return blockExScan;
}
...
#pragma omp parallel for
    for (uint i=0; i<numBlocks;i++) {
	std::vector<uint> local_offsets(numBuckets, 0);
	for (uint j=0; j<blockSize; j++) {
            uint k = (keys[blockSize*i+j] >> startBit) & (numBuckets-1);
	    sorted[blockExScan[k+i*numBuckets]+local_offsets[k]] = keys[blockSize*i+j];
	    local_offsets[k]++;
	}
    }
}
